...
The release tarball is **~420 KB**. Grab `tar-1.34-sco.tar.gz` from the
**Releases** page and extract it on the SCO box using the stock tooling:

```sh
# scp the tarball to the SCO machine, then on SCO:
...
- **GCC 3.4 or later** with C99 support. We tested with GCC 3.4.6 from
  `/asottile/prefix/bin`. The SCO-shipped GCC 2.95.3 may also work (no
  C99 features needed).
